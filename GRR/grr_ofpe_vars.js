...
for(j; j < years.length; j++){
  ...
  for(i; i < farms.length; i++){
    ...
    if(precCY==1){
      ...
      var currYear = CY;
      var startDate = PY + '-10-01';
      var endDate =  CY + '-09-30';
      ...
      Export.image.toDrive({
        ...
        description: current_farm + '_1km_daymet_' + currYear,
        folder:'GRR_GEE',
    ...
    if(gddCY==1){
      ...
      Export.image.toDrive({
        image: gdd_cy_clip, //
        description: current_farm + '_1km_daymet_gdd_' + grow_yr, 
        folder:'GRR_GEE',
        crs: 'EPSG:4326',
        scale: 10,
        region: mask_geometry
    ...
    if(precCY==1){
      ...
      Export.image.toDrive({
        image: precip_sum_cy_clipped,
        description: current_farm + '_4km_gridmet_prec_' + currYear,
        folder:'GRR_GEE',
        crs: 'EPSG:4326', // saves it as long lat wgs84 (reprojected for spatial analysis)
        scale: 10,
        region: mask_geometry
    ...
    if(gddCY==1){
      ...
      Export.image.toDrive({
        image: gdd_cy_clip, //
        description: current_farm + '_4km_gridmet_gdd_' + grow_yr, 
        folder:'GRR_GEE',
        crs: 'EPSG:4326',
        scale: 10,
        region: mask_geometry
